...
# load P files
pfile_name <- grep(pattern = glue::glue("{K}\\.P$"), value = T, x = list.files("data/ADMIXTURE/BEST_K/"))
pfile <- pophelper::readQ(files = paste0("data/ADMIXTURE/BEST_K/",pfile_name))[[1]]
...
write.table(x = treemix_input,
            file = glue::glue("data/ADMIXTURE/K-{K}_Outgroup={outgroup_population$Population[1]}=TREEMIX_input.txt"),
            quote = F,
            col.names = T,
            row.names = F)


treemix_input_name <- strsplit(glue::glue("data/ADMIXTURE/K-{K}_Outgroup={outgroup_population$Population[1]}=TREEMIX_input.txt"),
                               split = "/")[[1]][3]

# setwd(paste0(base.dir, "data/"))

system(glue::glue("gzip data/ADMIXTURE/{treemix_input_name}"))

outgroup_pop <- outgroup_population$Population[1]

output_name <- strsplit(strsplit(glue::glue("data/ADMIXTURE/K-{K}_Outgroup={outgroup_population$Population[1]}=TREEMIX_input.txt"),
                                 split = "/")[[1]][3], split = "\\.txt")[[1]][1]

system(glue::glue("treemix -i data/ADMIXTURE/{treemix_input_name}.gz -o data/ADMIXTURE/{output_name} -root {outgroup_pop} -k 500"))
...
system(paste0(paste0("printf \"", paste(LETTERS[1:K], collapse = '\n')), '\n\" > data/ADMIXTURE/poporder'))

treemix_stem <- glue::glue("data/ADMIXTURE/{output_name}")
...
plot_resid(stem = treemix_stem, "data/ADMIXTURE/poporder")
dev.off()
...
for(m in 1:4){
  system(glue::glue("treemix -i data/ADMIXTURE/{treemix_input_name}.gz -o data/ADMIXTURE/{output_name}_{m} -root {outgroup_pop} -k 500 -m {m} -se"))
}

# # # # #  PLOT TREEMIX WITH MIGRATIONS

for(migration in gsub("\\.llik","",grep(glue::glue("K-{K}"), grep("llik",list.files("data/ADMIXTURE/"),value = T), value = T))){
  
  system(paste0(paste0("printf \"", paste(LETTERS[1:K], collapse = '\n')), '\n\" > data/ADMIXTURE/poporder'))
  
  pdf(glue::glue("plots/TREEMIX_phylogeny_{migration}_migrations.pdf"))
  plot_tree(stem =  glue::glue("data/ADMIXTURE/{migration}"))
  dev.off()
  
  pdf(glue::glue("plots/TREEMIX_phylogeny_{migration}_migrations_RESIDUALS.pdf"))
  plot_resid(stem = glue::glue("data/ADMIXTURE/{migration}"), "data/ADMIXTURE/poporder")
  dev.off()
  # 
}
