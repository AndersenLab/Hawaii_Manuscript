...
admix_sharing <- hap_admix_df  %>%
  dplyr::filter(pop_assignment == "B" | Hawaiian == "TRUE") %>%
  distinct(isotype, chromosome, .keep_all= TRUE) %>%
  dplyr::group_by(isotype) %>%
  dplyr::mutate(genome_swept_hap_length = sum(isotype_swept_haplotype_length),
                genome_max_length = sum(max_swept_haplotype_length),
...
max_hap_sharing_plot_chrom_all_pops <- ggplot(admix_sharing_all) +
  ...
  geom_text(aes(label=ifelse(max_haplotype_shared > 0.15 & !pop_assignment %in% c("A", "B", "C"),
                             as.character(isotype),'')),hjust=1,vjust=.2, size = 2)
max_hap_sharing_plot_chrom_all_pops

ggsave(paste("plots/max_hap_sharing_plot_chr_all_pops.png"), width = 7.5, height = 7.5)
ggsave(paste("plots/max_hap_sharing_plot_chr_all_pops.pdf"), width = 7.5, height = 7.5)
...
ggsave(paste("plots/max_hap_sharing_plot_genome.png"), width = 3.75, height = 3.75)
ggsave(paste("plots/max_hap_sharing_plot_genome.pdf"), width = 3.75, height = 3.75)
...
admix_sharing_anlaysis <- admix_sharing %>%
  ...
  dplyr::mutate(pop_assignment = factor(pop_assignment, levels = c("B", "D", "E", "F")))
...
# plot occurance of max haplotype across genome
...
###### plot hawaii haplotypes by admix population using filtered sweep ratio
...
ggsave("plots/haplotype_admix_order.png", height = 5, width = 7.5)
ggsave("plots/haplotype_admix_order.pdf", height = 5, width = 7.5)
