...
# set directory for admix analysis
...
# load Q files
K <- 7
base.dir <- "~/Dropbox/AndersenLab/Hawaii_Manuscript/"
setwd(glue::glue("{base.dir}Data/ADMIXTURE/BEST_K/"))
qfile_name <- grep(pattern = glue::glue("LD_0.1_MAF_0.004.{K}.Q"), value = T, x = list.files())
...
admix_pops <- qfile[[1]] %>%
  ...
  dplyr::ungroup() %>%
  dplyr::mutate(Hawaiian = ifelse(isotype %in% hi_only_samples$samples, "TRUE", "FALSE"))

readr::write_tsv(admix_pops, "/Users/tim/Dropbox/AndersenLab/Hawaii_manuscript/data/ADMIXTURE/admix_assignments_K=7.tsv")
...
load("~/Dropbox/AndersenLab/Hawaii_manuscript/data/HAPLOTYPE/processed_haps.Rda")
...
# join admixture info
hap_admix_df <- dplyr::left_join(hap_df, admix_pops)

# reset working directory
setwd(glue::glue("{dirname(rstudioapi::getActiveDocumentContext()$path)}/.."))

# Plot % sharing of max haplotype between global admix population B and Hawaiian poulations E,F,D 
admix_sharing_all <- hap_admix_df  %>% #%>% dplyr::filter(pop_assignment == "B" | Hawaiian == "TRUE")
...
admix_sharing <- hap_admix_df  %>%
  dplyr::filter(pop_assignment == "A" | Hawaiian == "TRUE") %>%
  distinct(isotype, chromosome, .keep_all= TRUE) %>%
  dplyr::group_by(isotype) %>%
  dplyr::mutate(genome_swept_hap_length = sum(isotype_swept_haplotype_length),
                genome_max_length = sum(max_swept_haplotype_length),
...
max_hap_sharing_plot_chrom_all_pops <- ggplot(admix_sharing_all) +
  aes(x = pop_assignment, y = max_haplotype_shared, fill = pop_assignment) +
  scale_fill_manual(values=c(ancestry.colours)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.25, size = 1, shape = 21) +
  facet_wrap(~chromosome, scales = "free") +
  labs(y = "Fraction most common global haplotype", x = "Ancestral population") +
  theme_bw() +
  theme(legend.position="none") +
  geom_text(aes(label=ifelse(max_haplotype_shared > 0.05 & !pop_assignment %in% c("A", "B", "D", "E", "G"),
                             as.character(isotype),'')),hjust=1,vjust=.2, size = 2)
max_hap_sharing_plot_chrom_all_pops

ggsave(paste("plots/max_hap_sharing_plot_chr_all_pops_K=7.png"), width = 7.5, height = 7.5)
ggsave(paste("plots/max_hap_sharing_plot_chr_all_pops_K=7.pdf"), width = 7.5, height = 7.5)
...
max_hap_sharing_plot_chrom <- ggplot(admix_sharing) +
  ...
  theme_bw() +
  theme(legend.position="none") +
  geom_text(aes(label=ifelse(max_haplotype_shared > 0.15 & pop_assignment != "B",
                             as.character(isotype),'')),hjust=1,vjust=.2, size = 2)
...
max_hap_sharing_plot_genome <- ggplot(admix_sharing %>% dplyr::distinct(isotype, .keep_all=T)) +
  aes(x = pop_assignment, y = genome_frac_swept, fill = pop_assignment) +
  scale_fill_manual(values=c(ancestry.colours)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.25, size = 1, shape = 21) +
  ...
  theme_bw() +
  theme(legend.position="none") +
  geom_text(aes(label=ifelse(genome_frac_swept > 0.15 & pop_assignment != "B",as.character(isotype),'')),hjust=1,vjust=.2, size = 2)
max_hap_sharing_plot_genome

ggsave(paste("plots/max_hap_sharing_plot_genome_K=7.png"), width = 3.75, height = 3.75)
ggsave(paste("plots/max_hap_sharing_plot_genome_K=7.pdf"), width = 3.75, height = 3.75)
...
admix_sharing_anlaysis <- admix_sharing %>%
  ...
  dplyr::mutate(pop_assignment = factor(pop_assignment, levels = c("A", "C", "F")))
...
ggsave(paste("plots/admix_hi_fractions_K=7.png"), width = 3.75, height = 3.75)
ggsave(paste("plots/admix_hi_fractions_K=7.pdf"), width = 3.75, height = 3.75)
...
hap_admix_df_ordered <- hap_admix_df %>%
  dplyr::mutate(pop_assignment = ifelse(isotype == "XZ1515", "C", pop_assignment)) %>%
  dplyr::filter(Hawaiian == "TRUE" | pop_assignment == "A") %>%
  dplyr::arrange(desc(pop_assignment), plotpoint) %>%
  dplyr::mutate(pop_assignment = factor(pop_assignment, levels = c("A", "C",  "F")))

plotpoints <- hap_admix_df_ordered %>%
  dplyr::distinct(pop_assignment, isotype, plotpoint) %>%
  #dplyr::mutate(pop_assignment = ifelse(isotype == "XZ1515", "C", pop_assignment)) %>%
  dplyr::mutate(pop_assignment = factor(pop_assignment, levels = c("A", "C",  "F"))) %>%
  dplyr::arrange(pop_assignment, plotpoint) %>%
  dplyr::mutate(plotpoint_hi = row_number()) 

hap_admix_df_ordered <- dplyr::left_join(hap_admix_df_ordered, plotpoints) %>%
  dplyr::mutate(filter = ifelse(plotpoint_hi %in% seq(10, 90, by=1), TRUE, FALSE)) %>%
  dplyr::filter(filter != TRUE) %>%
  dplyr::mutate(plotpoint_hi = ifelse(plotpoint_hi > 10, plotpoint_hi-80, plotpoint_hi))
...
ggplot(hap_admix_df_ordered %>% dplyr::filter(Hawaiian == "TRUE"| pop_assignment == "A") %>% dplyr::mutate(type = ifelse(Hawaiian == "TRUE", "Hawaiian", "non-Hawaiian")),
  ...
  xlab("Position (Mb)") +
  theme_bw() +
  facet_grid(factor(pop_assignment, levels=c("F","C","A"))+type~chromosome, scales="free", space="free") +
  theme(legend.position="none")

ggsave("plots/haplotype_admix_order_K=7.png", height = 10, width = 10)
ggsave("plots/haplotype_admix_order_K=7.pdf", height = 10, width = 10)
...
ggplot(hap_admix_df_ordered %>% dplyr::filter(Hawaiian == "TRUE"),
  ...
  xlab("Position (Mb)") +
  theme_bw() +
  facet_grid(factor(pop_assignment, levels=c("F","C","A"))~chromosome, scales="free", space="free") +
  theme(legend.position="none")

ggsave("plots/haplotype_admix_order_grey_K=7.png", height = 5.5, width = 7.5)
ggsave("plots/haplotype_admix_order_grey_K=7.pdf", height = 5.5, width = 7.5)
