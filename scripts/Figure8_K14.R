...
#Set K=11 as admixture qfile
  k = 14
  qfile <- as.data.frame(pophelper::readQ("data/ADMIXTURE_LD8_K12/BEST_K/LD_0.8_MAF_0.004.14.Q"))
...
###  Generate stats for continuous variables
  ...
  admix <- data.table::fread("data/ADMIXTURE_LD8_K12/BEST_K/K14_Processed_Ancestry.tsv") %>%
  ...
  ass_stats_cont_dist_filtered_1 <- ass_stats_cont %>%
    dplyr::mutate(sub_pop2 = as.factor(ifelse(sub_pop %in% c("D","L"), "non_admixed", "Hawaiian Admixed"))) %>%
  ...
  k14_box_plots <- cowplot::plot_grid(elevation_p, sub_temp_p)
  k14_box_plots
  ggsave('plots/Hawaiian_population_association_box_plots_K=14_LD=8_25mfiltered_DLgrouped.pdf', width = 7.5, height = 5, useDingbats = F)
...
#5 plot HI map file supplement  
  ...
  ggsave(world_plot, filename = "plots/K14_HW_MAP_Admix.pdf",
         height = 10,
         width = 10)
...
#6 plot global populations
  ...
  full_world_plot <- ggplot()+ geom_map(data=world, map=world,
    ...
    geom_point(aes(long, lat, fill = pop_assignment),
               data=full_isolation_info %>%
                 dplyr::mutate(pop_assignment = factor(pop_assignment, level = c("A", "D", "E", "L", "M", "B",
                                                                                 "C", "F", "G", "H", "I", "J", "K", "N"))) %>%
  ...
  ggsave(full_world_plot, filename = "plots/K14_world_MAP_Admix.pdf", useDingbats = F,
         height = 7.5,
         width = 7.5)
...
# 7 plot euro map
  ...
  ggsave(euro_plot, filename = "plots/K14_euro_MAP_Admix.pdf", useDingbats = F,
        height = 3,
        width = 3)
