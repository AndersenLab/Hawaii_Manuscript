...
k_summary <- data.table::fread("data/ADMIXTURE/CV_Summary/admix_replicates_CV.tsv",header = T) 
...
# save
ggsave(ksummary_plot, filename = "plots/SuppFigureXX_KSummary.pdf", height = 8, width = 12)
ggsave(ksummary_plot, filename = "plots/SuppFigureXX_KSummary.png", height = 8, width = 12,dpi=300)
ggsave(final_plot, filename = "Plots/Figure6.png", height = 10, width = 12, dpi = 300)
ggsave(admix_legend, filename = "plots/admix_legend.pdf", height = 8, width = 12)
...
system(glue::glue("python scripts/vcf2phylip/vcf2phylip.py -i data/ANNOTATE_VCF/Hawaii.vcf.gz -m 43 --fasta --nexus --nexus-binary"))
...
for(apops in unique(representative_K_strains$K_size)){
  ...
  # Generate nexus file, .nexus file is used for SplitsTree
  system(glue::glue("python scripts/vcf2phylip/vcf2phylip.py -i data/ANNOTATE_VCF/Hawaii_K{apops}.vcf.gz -m {n_strains} --fasta --nexus --nexus-binary"))
}
