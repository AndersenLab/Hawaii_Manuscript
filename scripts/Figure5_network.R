...
ggsave('plots/Figure5B_caenorhabditis.pdf', width = 5, height = 7)
...
isotypes_found_together <- cso%>%
  dplyr::filter(c_label %in% cso_isotypes$c_label)%>%
  dplyr::select(isotype, c_label, s_label)%>%
  dplyr::filter(!is.na(isotype))%>%
  dplyr::group_by(c_label)
...
ggsave('plots/Figure5D_isotypes.pdf', width = 5, height = 7)

#isotypes_found_together$isotype <- gsub(" ", 
                                      #"\n",
                                      #species_found_together$isotype)
# count number of instances C. elegans is found with other types on same substrate
ce_same_sub <- cso %>%
