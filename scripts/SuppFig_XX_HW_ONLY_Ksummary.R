...
k_summary <- data.table::fread("data/ADMIXTURE_LD8_HW_ONLY/CV_Summary/admix_replicates_CV.tsv",header = T) 
...
admix_plots <- list()
for(kpops in 1:length(grep(".Q", list.files("data/ADMIXTURE_LD8_HW_ONLY/BEST_K/"), value = T))){
  K <- as.numeric(strsplit(grep(".Q", list.files("data/ADMIXTURE_LD8_HW_ONLY/BEST_K/"), value = T)[kpops], split = "\\.")[[1]][4])
  
  # load Q files
  qfile_name <- grep(pattern = glue::glue("{K}\\.Q$"), value = T, x = list.files("data/ADMIXTURE_LD8_HW_ONLY/BEST_K/"))
  qfile <- pophelper::readQ(files = paste0("data/ADMIXTURE_LD8_HW_ONLY/BEST_K/",qfile_name))[[1]]
  # add pop names
  ...
  write.table(qfile, file = glue::glue("data/ADMIXTURE_LD8_HW_ONLY/BEST_K/K{K}_Processed_Ancestry.tsv"), col.names = T, row.names = F, quote = F, sep = "\t")
  ...
  ggsave(admix_plots[[kpops]]  +
           theme(axis.text.x=element_text(angle = 90)), filename = glue::glue("plots/SuppFigureXX_K{K}_LD8_HI_ONLY_names.pdf"), height = 4, width = 28)
  ggsave(admix_plots[[kpops]]  +
           theme(axis.text.x=element_text(angle = 90)), filename = glue::glue("plots/SuppFigureXX_K{K}_LD8_HI_ONLY_names.pdf"), height = 4, width = 28,dpi=300)
}
...
# save
ggsave(ksummary_plot, filename = "plots/SuppFigureXX_KSummary_LD8_HI_ONLY_.pdf", height = 8, width = 12, useDingbats=FALSE)
ggsave(ksummary_plot, filename = "plots/SuppFigureXX_KSummary_LD8_HI_ONLY_.png", height = 8, width = 12,dpi=300)
ggsave(admix_legend, filename = "plots/admix_legend_LD8_HI_ONLY_.pdf", height = 8, width = 12)
