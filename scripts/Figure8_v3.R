...
# stats on island and substrate asociations for isotypes X isotypes from XX number of distinct samples
# load admix info for full 276_set
admix <- data.table::fread("~/Dropbox/AndersenLab/Hawaii_manuscript/data/ADMIXTURE/admix_assignments_K=6.tsv")
...
# box plots showing with test from above K=6 global
...
k6_box_plots <- cowplot::plot_grid(altitude_small, sub_temp_small)
k6_box_plots
ggsave('plots/K=6_global_population_association_box_plots.pdf', width = 7.5, height = 7.5)
...
ass_sub<- ass_stats %>%
  ...
  dplyr::mutate(d = ifelse(pop_assignment == "D", 1, NA),
                e = ifelse(pop_assignment == "E", 1, NA),
                f = ifelse(pop_assignment == "F", 1, NA)) %>%
  dplyr::group_by(fixed_substrate, pop_assignment) %>%
  dplyr::mutate(d = na.pass(sum(d)),
                e = na.pass(sum(e)),
                f = na.pass(sum(f))) %>%
  dplyr::ungroup()
...
ass_sub_d <- ass_sub %>%
  ...
  dplyr::mutate(no_d = tot_sub_num-d,
                d = d) %>%
  dplyr::select(fixed_substrate, no_d, d) %>%
  dplyr::distinct(fixed_substrate, .keep_all=TRUE) %>%
  column_to_rownames(var = "fixed_substrate") %>%
  as.matrix(.)

ass_sub_e <- ass_sub %>%
  dplyr::filter(pop_assignment == "E") %>%
  dplyr::mutate(no_e = tot_sub_num-e,
                e = e) %>%
  dplyr::select(fixed_substrate, no_e, e) %>%
  dplyr::distinct(fixed_substrate, .keep_all=TRUE) %>%
  column_to_rownames(var = "fixed_substrate") %>%
  as.matrix(.)
...
chisq.test(ass_sub_d, simulate.p.value = TRUE)

# full chi squared test to see if there are differernces 
chisq.test(ass_sub_e, simulate.p.value = TRUE)
...
ass_isl_d <- ass_isl %>%
  dplyr::filter(pop_assignment == "D") %>%
  dplyr::mutate(no_d = tot_isl_num-d,
                d = d) %>%
  dplyr::select(island, no_d, d) %>%
...
ass_isl_e <- ass_isl %>%
  dplyr::filter(pop_assignment == "E") %>%
  dplyr::mutate(no_e = tot_isl_num-e,
                e = e) %>%
  dplyr::select(island, no_e, e) %>%
  dplyr::distinct(island, .keep_all=TRUE) %>%
  column_to_rownames(var = "island") %>%
  as.matrix(.)
...
ass_isl_f <- ass_isl %>%
  dplyr::filter(pop_assignment == "F") %>%
  dplyr::mutate(no_f = tot_isl_num-f,
                f = f) %>%
...
# full chi squared test to see if there are differernces 
chisq.test(ass_isl_d, simulate.p.value = TRUE)

pairwiseNominalIndependence(ass_isl_d,
                            fisher = TRUE,
                            gtest  = FALSE,
                            chisq  = FALSE,
...
# full chi squared test to see if there are differernces 
chisq.test(ass_isl_e, simulate.p.value = TRUE)

# full chi squared test to see if there are differernces 
chisq.test(ass_isl_f, simulate.p.value = TRUE)

#############################################################
# Same Stats with Hawaii only Q-file (4 hawaiian populations)
iso_names <- data.table::fread("~/Dropbox/AndersenLab/Hawaii_manuscript/data/ADMIXTURE/Local_global_stats/all_hawaii.txt", header = F)
admix_4 <- data.table::fread("/Users/tim/Dropbox/AndersenLab/Hawaii_manuscript/data/ADMIXTURE/Local_global_stats/LD_0.95_MAF_0.025_4_1499.Q") %>%
  dplyr::mutate(isotype = iso_names$V1) %>%
  dplyr::rename(A = V1, B = V2, C = V3, D = V4) %>%
  tidyr::gather(pop_assignment, value, -isotype) %>%
  dplyr::group_by(isotype) %>%
  dplyr::mutate(max_frac = max(value)) %>%
  dplyr::filter(value == max_frac)

# load data frame to do stats, may need to manually run code lines in loop above to get hm_hi5 df
ass_stats_4 <- left_join(hm_hi5, cso %>% dplyr::filter(!is.na(isotype)) %>% dplyr::select(s_label, grid_num)) %>%
  dplyr::filter(!is.na(isotype)) %>%
  dplyr::distinct(c_label, isotype, .keep_all = TRUE) # could filter distinct gridsect to reduce effect of over sampling at those locations

# join admix and ass_stats
ass_stats_4 <- left_join(ass_stats_4, admix_4) %>%
  group_by(c_label) %>%
  dplyr::mutate(c_label2 = as.character(ifelse(is.na(c_label), row_number(), c_label))) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(c_label = c_label2)
...
ass_stats_cont_small_grid_4 <- ass_stats_4 %>%
  dplyr::select(lat = latitude, long = longitude,
                c_label, grid_num, fixed_substrate, isotype, sub_pop = pop_assignment, altitude,
                ambient_temperature, substrate_temperature, ambient_humidity, substrate_moisture) %>%
  dplyr::mutate(sub_pop = factor(sub_pop)) %>%
...
ass_stats_cont_small_nogrid_4 <- ass_stats_4 %>%
  dplyr::select(lat = latitude, long = longitude,
                c_label, grid_num, fixed_substrate, isotype, sub_pop = pop_assignment, altitude,
                ambient_temperature, substrate_temperature, ambient_humidity, substrate_moisture) %>%
  dplyr::mutate(sub_pop = factor(sub_pop)) %>%
...
ass_stats_cont_small_4 <- full_join(ass_stats_cont_small_nogrid_4, ass_stats_cont_small_grid_4) %>%
  tidyr::gather(env_par, value, -lat, -long, -c_label, -grid_num, -fixed_substrate, -isotype, -sub_pop)
...
for (e in 1:length(unique(ass_stats_cont_4$env_par))){
  KM_df <- ass_stats_cont_4 %>%
    dplyr::filter(env_par == (unique(ass_stats_cont_4$env_par)[e]))
  
  D_test <- dunnTest(KM_df$value ~ KM_df$sub_pop, method = "bonferroni") 
  Dunn_list_4[[unique(ass_stats_cont_4$env_par)[e]]] <- D_test
}

Dunn_list_4

# box plot
ggplot(ass_stats_cont_4 %>% tidyr::spread(env_par, value)) +
  aes(x = sub_pop, y = altitude) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(wdith = 0.25, alpha = 0.5, shape = 21)
...
for (e in 1:length(unique(ass_stats_cont_small_4$env_par))){
  KM_df <- ass_stats_cont_small_4 %>%
    dplyr::filter(env_par == (unique(ass_stats_cont_small_4$env_par)[e]))
  
  D_test <- dunnTest(KM_df$value ~ KM_df$sub_pop, method = "bonferroni") 
  Dunn_list[[unique(ass_stats_cont_small_4$env_par)[e]]] <- D_test
}

Dunn_list

# box plot for K=4 small
altitude_small4 <- ggplot(ass_stats_cont_small_4 %>% tidyr::spread(env_par, value)) +
  aes(x = sub_pop, y = altitude) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(wdith = 0.25, alpha = 0.5) +
  theme_bw()
...
sub_temp_small4 <- ggplot(ass_stats_cont_small_4 %>% tidyr::spread(env_par, value)) +
  aes(x = sub_pop, y = substrate_temperature) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(wdith = 0.25, alpha = 0.5) +
  theme_bw()
...
k4_box_plots <- cowplot::plot_grid(altitude_small4, sub_temp_small4)

ggsave('plots/K=4_HI_population_association_box_plots.pdf', width = 7.5, height = 7.5)

# do fisher exact test for substrate enrichment 
ass_sub_4<- ass_stats_4 %>%
  dplyr::distinct(isotype, grid_num, fixed_substrate, .keep_all = T) %>%
  dplyr::group_by(fixed_substrate) %>%
  dplyr::mutate(tot_sub_num = n()) %>%
  dplyr::mutate(a = ifelse(pop_assignment == "A", 1, NA),
                b = ifelse(pop_assignment == "B", 1, NA),
                c = ifelse(pop_assignment == "C", 1, NA),
                d = ifelse(pop_assignment == "D", 1, NA)) %>%
  dplyr::group_by(fixed_substrate, pop_assignment) %>%
  dplyr::mutate(a = na.pass(sum(a)),
                b = na.pass(sum(b)),
                c = na.pass(sum(c)),
                d = na.pass(sum(d))) %>%
  dplyr::ungroup()

ass_sub_4a <- ass_sub_4 %>%
  dplyr::filter(pop_assignment == "A") %>%
  dplyr::mutate(no_a = tot_sub_num-a,
                a = a) %>%
  dplyr::select(fixed_substrate, no_a, a) %>%
  dplyr::distinct(fixed_substrate, .keep_all=TRUE) %>%
  column_to_rownames(var = "fixed_substrate") %>%
  as.matrix(.)

ass_sub_4b <- ass_sub_4 %>%
  dplyr::filter(pop_assignment == "B") %>%
  dplyr::mutate(no_b = tot_sub_num-b,
                b = b) %>%
  dplyr::select(fixed_substrate, no_b, b) %>%
  dplyr::distinct(fixed_substrate, .keep_all=TRUE) %>%
  column_to_rownames(var = "fixed_substrate") %>%
  as.matrix(.)

ass_sub_4c <- ass_sub_4 %>%
...
# full chi squared test to see if there are differernces 
chisq.test(ass_sub_4a, simulate.p.value = TRUE)

pairwiseNominalIndependence(ass_sub_4a,
                            fisher = TRUE,
                            gtest  = FALSE,
                            chisq  = FALSE,
                            method = "bonferroni", simulate.p.value = TRUE)
...
# full chi squared test to see if there are differernces 
chisq.test(ass_sub_4b, simulate.p.value = TRUE)

# full chi squared test to see if there are differernces 
chisq.test(ass_sub_4c, simulate.p.value = TRUE)

# full chi squared test to see if there are differernces 
chisq.test(ass_sub_4d, simulate.p.value = TRUE)

# do fisher exact test for Island enrichment with K = 4 Hawaii
ass_isl_4<- ass_stats_4 %>%
  dplyr::distinct(isotype, grid_num, island, .keep_all = T) %>%
  dplyr::group_by(island) %>%
  dplyr::mutate(tot_isl_num = n()) %>%
  dplyr::mutate(a = ifelse(pop_assignment == "A", 1, NA),
...
ass_isl_4a <- ass_isl_4 %>%
  dplyr::filter(pop_assignment == "A") %>%
  dplyr::mutate(no_a = tot_isl_num-a,
                a = a) %>%
  dplyr::select(island, no_a, a) %>%
  dplyr::distinct(island, .keep_all=TRUE) %>%
  column_to_rownames(var = "island") %>%
  as.matrix(.)

ass_isl_4b <- ass_isl_4 %>%
  dplyr::filter(pop_assignment == "B") %>%
  dplyr::mutate(no_b = tot_isl_num-b,
                b = b) %>%
  dplyr::select(island, no_b, b) %>%
  dplyr::distinct(island, .keep_all=TRUE) %>%
  column_to_rownames(var = "island") %>%
  as.matrix(.)

ass_isl_4c <- ass_isl_4 %>%
  dplyr::filter(pop_assignment == "C") %>%
  dplyr::mutate(no_c = tot_isl_num-c,
                c = c) %>%
...
# full chi squared test to see if there are differernces 
chisq.test(ass_isl_4a, simulate.p.value = TRUE)

pairwiseNominalIndependence(ass_isl_4a,
                            fisher = TRUE,
                            gtest  = FALSE,
                            chisq  = FALSE,
                            method = "bonferroni", simulate.p.value = TRUE)
...
# full chi squared test to see if there are differernces 
chisq.test(ass_isl_4b, simulate.p.value = TRUE)

# full chi squared test to see if there are differernces 
chisq.test(ass_isl_4d, simulate.p.value = TRUE)

pairwiseNominalIndependence(ass_isl_4d,
                            fisher = TRUE,
                            gtest  = FALSE,
                            chisq  = FALSE,
                            method = "bonferroni", simulate.p.value = TRUE)

# island enrichment test for K = 6
ass_isl<- ass_stats %>%
  dplyr::distinct(isotype, grid_num, island, .keep_all = T) %>%
  dplyr::group_by(island) %>%
  dplyr::mutate(tot_isl_num = n()) %>%
