...
######################
# setup plot for supplemental figure
pop_size_by_sp <- pop_size_caen_by_sp_no_multiples %>%
  dplyr::group_by(species_family) %>%
  dplyr::mutate(sp_total = sum(count_per_size)) %>%
  dplyr::ungroup() %>%
  dplyr::mutate(perc_in_size_group = (count_per_size/sp_total)*100)
  
# plot fraction of sp per popsize
ggplot(pop_size_by_sp) +
  aes(x = fixed_approximate_number_of_worms, y = perc_in_size_group, fill = species_family) +
